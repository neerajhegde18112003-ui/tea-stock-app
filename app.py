...
# PASTE YOUR GOOGLE SHEET SHARE LINK INSIDE THE QUOTES BELOW:
GOOGLE_SHEET_URL = "https://docs.google.com/spreadsheets/d/11rOF-i3498T3Jyz2SOlBt4oNDdBerGX0mRYPfqsyif8/edit?usp=sharing"

if GOOGLE_SHEET_URL == "YOUR_GOOGLE_SHEET_SHARE_LINK_HERE":
    st.error("⚠️ Setup Missing: Please paste your copied Google Sheets URL into line 33 of app.py!")
    st.stop()

# Initialize the secure sheets connection layer
conn = st.connection("gsheets", type=GSheetsConnection)

# Helper function to fetch latest live data from Google Sheets
def load_inventory():
    df = conn.read(spreadsheet=GOOGLE_SHEET_URL, worksheet="Stock", ttl="0s")
    # Convert dataframe format into our handy dictionary format
    inventory = {}
...
# Helper function to push data modifications back to Google Sheets
def save_inventory(updated_inventory):
    rows = []
    for item_name, data in updated_inventory.items():
        rows.append({
            "item_name": item_name,
            "stock": data["stock"],
            "price": data["price"],
    ...
    conn.update(spreadsheet=GOOGLE_SHEET_URL, worksheet="Stock", data=df)
    st.cache_data.clear()

# Load fresh data into application state memory
...
with st.expander("➕ Add New Tea Variety to Inventory", expanded=False):
    ...
    if st.button("Add to Dashboard ✨", use_container_width=True):
        ...
        elif new_item_name in current_inventory:
            st.error("This variety already exists!")
        else:
